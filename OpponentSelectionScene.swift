//
//  OpponentSelectionScene.swift
//  TVRumbleTest1
//
//  Created by Nicolas Mariniello && Fabio Friano on 04/04/2020.
//

import SpriteKit
import AVFoundation

class OpponentSelectionScene: SKScene {
    var mute = false
    var audioPlayer = AVAudioPlayer()
    var selection = ""
    var characters = [String]()

    private var labelToBeChanged = SKLabelNode()
    private var labelToBeChanged1 = SKLabelNode()
    ...
    private var fightButton = SKSpriteNode()
    
    override func didMove(to view: SKView) {
        labelToBeChanged = childNode(withName: "labelToBeChanged") as! SKLabelNode
        labelToBeChanged.text = "SELECT YOUR OPPONENT"
        labelToBeChanged1 = childNode(withName: "labelToBeChanged1") as! SKLabelNode
        labelToBeChanged1.text = "FIGHT!"
        ...
        fightButton = childNode(withName: "nextButton") as! SKSpriteNode
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if previousCharacter.contains(touchLocation) {
        ...
        } else if fightButton.contains(touchLocation){
            removeAction(forKey: "go")
            
            let versusScene = VersusScene(fileNamed: "VersusScene")
            versusScene?.scaleMode = .aspectFit
            versusScene?.label1 = selection
            versusScene?.label2 = characters[selectedIndex]
            versusScene?.audioPlayer = audioPlayer
            
            self.view?.presentScene(versusScene!, transition: SKTransition.doorsCloseHorizontal(withDuration: 1))
            ...
            let characterSelectionScene = CharacterSelectionScene(fileNamed: "CharacterSelectionScene")
            characterSelectionScene?.scaleMode = .aspectFit
            characterSelectionScene?.mute = mute
            characterSelectionScene?.audioPlayer = audioPlayer
            
            self.view?.presentScene(characterSelectionScene!, transition: SKTransition.push(with: .right, duration: 1))
        }
    }
